...
RETURNS TRIGGER AS $$
BEGIN
    -- Only generate OTP if NOT a COD order
    IF NEW.delivery_otp IS NULL AND NEW.payment_method != 'cod' THEN
...
-- Cleanup: Clear OTP for existing COD orders that are not yet delivered
