...
// Hardcoding from the .env file that we read earlier, ensuring NO truncation
...
async function run() {
    ...
    const pIds = new Set(profiles.map((p: any) => p.user_id));
    ...
    const orphans = users.filter((u: any) => !pIds.has(u.id));
