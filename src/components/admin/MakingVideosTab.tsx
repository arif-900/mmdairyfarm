  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-end pb-8 border-b border-forest/10">
        ...
        <Button onClick={() => handleOpenDialog()} className="bg-forest hover:bg-forest/90 text-white rounded-2xl h-12 px-8 font-black uppercase text-xs tracking-widest shadow-xl shadow-forest/10 transition-all active:scale-95">
      ...
      <div className="space-y-6">
        ...
        {bucketError && (
          <div className="bg-amber-50 border border-amber-200 rounded-[24px] p-6 flex flex-col md:flex-row items-center justify-between gap-4 animate-slide-up shadow-sm">
            ...
            <Button variant="outline" size="sm" onClick={checkBucket} className="rounded-xl border-amber-200 text-amber-900 font-black h-10 px-6 uppercase text-[10px] tracking-widest hover:bg-amber-100">
              Refresh Setup
            </Button>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="bg-white/30 backdrop-blur-sm border-2 border-dashed border-forest/10 rounded-[32px] p-20 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {videos.map((video) => (
              <Card key={video.id} className={`group overflow-hidden rounded-[32px] border border-forest/5 shadow-soft transition-all duration-700 hover:shadow-2xl hover:-translate-y-2 ${!video.isActive ? "opacity-50 grayscale" : ""}`}>
                ...
                <CardFooter className="p-6 pt-0 flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-between w-full">
                    ...
                    <div className="flex gap-2">
                      <Button variant="ghost" size="icon" onClick={() => handleOpenDialog(video)} className="h-10 w-10 rounded-xl hover:bg-forest/5 hover:text-forest transition-colors">
                        <Edit className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleDelete(video)} className="h-10 w-10 rounded-xl hover:bg-rose-50 hover:text-rose-500 transition-colors">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px] p-0 rounded-[32px] overflow-hidden border-none shadow-3xl bg-white/95 backdrop-blur-2xl max-h-[95vh] flex flex-col">
          <DialogHeader className="p-6 pb-5 bg-forest text-white relative overflow-hidden flex-shrink-0">
            ...
            <DialogTitle className="text-3xl font-display font-black italic uppercase tracking-tighter flex items-center gap-4 relative z-10">
              <div className="p-2.5 bg-golden/20 rounded-2xl">
          ...
          <div className="p-7 space-y-6 overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-3">
              ...
              <Textarea
                ...
                className="rounded-2xl min-h-[120px] bg-white border-forest/10 focus:border-forest/30 focus:ring-forest/5 shadow-inner p-4 text-sm leading-relaxed"
              />
            </div>

            <div className="space-y-5">
              <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-forest/40 ml-1">Video Source</Label>
              <div className="flex rounded-2xl bg-forest/5 p-1.5 border border-forest/10">
                <Button
                  type="button"
                  variant={currentVideo.type === 'url' ? 'default' : 'ghost'}
                  onClick={() => setCurrentVideo({ ...currentVideo, type: 'url' })}
                  className="flex-1 rounded-xl font-bold h-10 text-[11px] uppercase tracking-wider"
                >
                ...
                <Button
                  ...
                  className={`flex-1 rounded-xl font-black h-10 text-[10px] uppercase tracking-widest transition-all ${currentVideo.type === 'upload' ? 'bg-forest text-white shadow-lg' : 'text-forest/40 hover:text-forest'}`}
                >
              ...
              {currentVideo.type === 'url' ? (
                ...
                <div className="space-y-3">
                  <div className="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:border-forest/40 transition-colors cursor-pointer relative">
                    <input
                      type="file"
                      accept="video/*"
          ...
          <DialogFooter className="p-6 pt-4 bg-slate-50 border-t flex-shrink-0">
            <Button variant="ghost" onClick={() => setIsDialogOpen(false)} disabled={saving} className="rounded-xl font-black text-[10px] uppercase tracking-widest">Discard</Button>
            <Button onClick={handleSave} disabled={saving || isUploading} className="rounded-2xl h-12 px-8 bg-forest hover:bg-forest/90 text-white font-black text-xs uppercase tracking-[0.2em] shadow-xl border-b-4 border-forest-dark active:border-b-0 active:translate-y-1 transition-all">
