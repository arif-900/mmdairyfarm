      ...
      <DialogContent aria-describedby={undefined} className="max-w-2xl max-h-[90vh] overflow-y-auto p-0 border-none bg-cream shadow-elevated">
        <DialogHeader className="p-8 border-b border-forest/10 relative overflow-hidden bg-white/50 backdrop-blur-md">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 bg-forest/10 rounded-2xl flex items-center justify-center border border-forest/20 shadow-inner">
            ...
            <div className="flex items-center gap-2">
              <Button 
                ...
                className="gap-2 rounded-xl h-10 border-forest/10 font-black uppercase text-[10px] tracking-widest hover:bg-forest/5"
                disabled={loading}
              >
        ...
        <div className="space-y-8 mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white/40 border border-forest/10 rounded-2xl p-5 shadow-soft relative overflow-hidden group">
              ...
              <div className="space-y-4 text-sm relative z-10">
                <div className="flex items-center gap-3 bg-white/60 p-2.5 rounded-xl border border-white/50">
                  <div className="p-1.5 bg-forest/10 rounded-lg text-forest">
                    <User className="h-3 w-3" />
                  </div>
                ...
                <div className="flex items-start gap-3 bg-white/60 p-2.5 rounded-xl border border-white/50">
                  <div className="p-1.5 bg-forest/10 rounded-lg text-forest mt-0.5">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" className="flex-1 rounded-xl h-9 font-black uppercase text-[9px] tracking-widest border-forest/10 hover:bg-forest/5" asChild>
            ...
            {/* Payment & Status Card */}
            <div className="bg-white/40 border border-forest/10 rounded-2xl p-5 shadow-soft relative group">
              <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:scale-110 transition-transform">
                ...
                {((order as any).razorpay_payment_id || (order as any).refund_id) && (
                  <div className="pt-3 border-t border-forest/5 flex flex-col gap-1.5">
                    ...
                    {(order as any).refund_id && (
                      <div className="flex items-center gap-2 mt-1 p-2 bg-blue-50/50 rounded-lg border border-blue-100">
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              <div className="flex justify-center py-8 bg-white/30 rounded-2xl border border-forest/5">
                <Loader2 className="h-8 w-8 animate-spin text-forest/40" />
              </div>
            ) : fetchError ? (
              <div className="border border-red-200 bg-red-50/50 rounded-2xl p-4 text-red-700 text-xs font-bold uppercase tracking-tight">
                Error sync: {fetchError}
              </div>
            ) : (
              <div className="grid gap-3">
                {items.map((item) => (
                  <div key={item.id} className="p-4 bg-white/60 backdrop-blur-sm rounded-2xl flex justify-between items-center border border-white/80 shadow-soft group hover:border-forest/20 transition-all">
          ...
          <div className="bg-white/40 border border-forest/10 rounded-2xl p-6 shadow-soft space-y-4">
            <h3 className="font-black text-forest uppercase text-xs tracking-widest flex items-center gap-2">
                   ...
                   {order.payment_method === 'cod' ? (
                    <div className="flex flex-col items-end gap-2">
                       ...
                       <Button 
                        ...
                        disabled={updating}
                        className={order.is_cash_collected ? "bg-emerald-600 hover:bg-emerald-700 h-9 rounded-xl px-4" : "border-amber-400 text-amber-700 hover:bg-amber-50 h-9 rounded-xl px-4 font-black text-[9px] uppercase tracking-widest"}
                      >
          ...
          <div className="bg-forest/5 border border-forest/10 rounded-2xl p-6 space-y-4">
            <h3 className="font-black text-forest uppercase text-xs tracking-widest flex items-center gap-2">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                <SelectTrigger className="flex-1 h-12 rounded-xl bg-white border-forest/10 font-bold text-sm text-forest">
                  <SelectValue placeholder="Update Status" />
                </SelectTrigger>
                <SelectContent className="rounded-xl border-forest/10">
              ...
              <Button 
                ...
                className="h-12 px-8 rounded-xl bg-forest hover:bg-forest-dark text-white font-black uppercase text-[10px] tracking-widest transition-all"
            ...
            {isAdmin && order.status !== 'cancelled' && ['pending', 'paid', 'processing'].includes(order.status) && (
              <Button 
                variant="outline" 
                className="w-full h-11 rounded-xl gap-2 font-black uppercase text-[9px] tracking-[0.2em] border-rose-100 text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition-all shadow-none" 
