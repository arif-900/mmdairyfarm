...
export function DeliveryTrackingTab() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-emerald-50/50 p-6 rounded-[10px] border border-emerald-100">
        <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-emerald-600 rounded-[10px] flex items-center justify-center text-white shadow-lg shadow-emerald-600/20">
        ...
        <div className="flex items-center gap-3 bg-white p-2 rounded-[10px] border border-emerald-100 shadow-sm">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Select Date:</p>
            <Input 
                type="date"
                value={selectedDate}
                onChange={(e) => setSelectedDate(e.target.value)}
                className="h-10 border-none bg-emerald-50/50 rounded-[10px] font-bold text-emerald-700 focus-visible:ring-0 w-40"
            />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1 border-none shadow-xl rounded-[10px] overflow-hidden bg-slate-900 text-white">
            <CardContent className="p-6">
                ...
                <div className="space-y-4">
                    {Object.entries(productSummary).map(([name, qty]: any) => (
                        <div key={name} className="flex justify-between items-center bg-white/5 p-4 rounded-[10px] hover:bg-white/10 transition-colors">
        ...
        {/* Rider Progress */}
        <Card className="lg:col-span-2 border-none shadow-xl rounded-[10px] overflow-hidden bg-white">
            <CardContent className="p-6">
                <div className="flex items-center gap-3 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {riderPerformance.map(r => (
                        <div key={r.name} className="bg-slate-50 p-4 rounded-[10px] border border-slate-100">
      ...
      {/* Control Bar */}
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-[10px] border border-slate-100 shadow-sm">
        <div className="relative flex-1 w-full">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
          <Input 
            placeholder="Search customer or phone..." 
            className="pl-10 h-12 rounded-[10px] border-slate-100 bg-slate-50 focus:bg-white transition-all shadow-inner"
            value={searchTerm}
        ...
        <div className="flex gap-2 w-full md:w-auto">
          <Select value={slotFilter} onValueChange={setSlotFilter}>
            <SelectTrigger className="w-[140px] h-12 rounded-[10px] bg-slate-50 border-slate-100 font-bold uppercase text-[10px] tracking-widest">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-[140px] h-12 rounded-[10px] bg-slate-50 border-slate-100 font-bold uppercase text-[10px] tracking-widest text-emerald-600">
          ...
          <Button variant="outline" onClick={fetchData} className="h-12 w-12 rounded-[10px] p-0 hover:bg-emerald-50 text-emerald-600 border-emerald-100 shadow-sm">
      ...
      <div className="bg-white rounded-[10px] border border-slate-100 shadow-xl overflow-hidden">
                 ...
                 <tr key={drop.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="p-5">
                       <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-[10px] bg-emerald-50 flex items-center justify-center text-emerald-600">
                    ...
                    <td className="p-5">
                       <Badge variant="outline" className={cn(
                         "uppercase text-[9px] font-black tracking-widest px-2 py-1 rounded-[10px] border-slate-100",
                    ...
                    <td className="p-5">
                       <div className="flex flex-col gap-1">
                          ...
                          {drop.status === 'skipped' && drop.notes && (
                            <p className="text-[9px] font-bold text-rose-500 bg-rose-50 px-2 py-0.5 rounded-[10px] border border-rose-100 max-w-[140px] truncate" title={drop.notes}>
                              Reason: {drop.notes}
                            </p>
                          )}
                       </div>
                    ...
                    <td className="p-5 text-right">
                       ...
                       >
                          <SelectTrigger className={cn(
                            "w-[180px] h-10 rounded-[10px] font-bold ml-auto transition-all",
