...
export default function Stepper({
  ...
  // Support controlled mode if needed, but for now we'll stick to internal state 
  // and sync it with initialStep if it changes.
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: StepIndicatorProps) {
  ...
  const completeColor = "hsl(var(--forest-light))";
  ...
  return (
    ...
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'white', color: 'hsl(var(--muted-foreground))', borderColor: 'hsl(var(--border))' },
          active: { scale: 1.1, backgroundColor: activeColor, color: 'white', borderColor: activeColor },
          complete: { scale: 1, backgroundColor: completeColor, color: 'white', borderColor: completeColor }
        }}
